...
if not lspconfig_status then
    vim.notify("Unable to require lspconfig", vim.lsp.log_levels.ERROR, {title = "Plugin error"})
    return
end
...
local function lsp_keymaps(bufnr)
    ...
    if tsbuildinok then
        vim.keymap.set("n", "gr", function() tsbuildin.lsp_references() end, {noremap = true, desc = "Show references"})
    end

    local go, _ = pcall(require, "go")
    if go then
        vim.keymap
            .set("n", "<Leader>ca", ":GoCodeAction<CR>", {silent = true, noremap = true, desc = "Show code actions"})
    end
    ...
    vim.keymap
        .set("n", "<Leader>rn", function() vim.lsp.buf.rename() end, {noremap = true, desc = "[R]e[n]ame via LSP"})
...
local on_attach = function(client, bufnr)
    ...
    if client.name ~= "pylsp" and client.name ~= "null-ls" and client.name ~= "efm" then navic.attach(client, bufnr) end
